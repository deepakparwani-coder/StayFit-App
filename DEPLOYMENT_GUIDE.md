...
Open `index.html` in a text editor. Near the top, find the `firebaseConfig` block:
...
Replace all six `REPLACE_*` values with the ones you copied from Firebase in Step 1. Save the file.

That's the only code change needed. **The trainer PIN is no longer hardcoded** — the first person to open the app sets it through an in-app setup screen, and it can be changed anytime from **Settings (⚙️) → Change PIN** inside the trainer view.

> **Security note:** The Firebase API key is visible in the source — that's by design (security is enforced by Firestore rules, not by hiding the keys). The trainer PIN is stored in the database; access is gated by Firebase Auth + Firestore rules. It's a soft barrier — fine for keeping members out of the trainer view, but not a fortress. For stronger security we'd add proper email/password auth.
...
**First time only:** open the app → "I'm the Trainer" → you'll see a "Set up your PIN" screen. Create a 4–8 digit PIN you'll remember. This becomes the trainer-view password for all devices.
...
**To change your PIN later:** tap the ⚙️ gear icon in the trainer header → Change Trainer PIN. You'll need to enter your current PIN first.
...
**"Incorrect PIN"** → Open the app → tap "I'm the Trainer" → there's a "Forgot your PIN?" expandable note with instructions. The short version: go to Firebase Console → Firestore → `settings` collection → delete the `trainer_pin` document. Then come back to the app and you'll be prompted to set a new PIN.
